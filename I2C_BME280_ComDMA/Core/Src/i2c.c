...
int8_t i2c_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len,
	...
	status = HAL_I2C_Master_Transmit_DMA(&hi2c1, dev_addr << 1, &reg_addr, 1);
	if (status != HAL_OK) {
		return status;
	}
	...
	status = HAL_I2C_Master_Receive_DMA(&hi2c1, dev_addr << 1, reg_data, len);
	if (status != HAL_OK) {
		return status;
	}
...
int8_t i2c_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t len,
	...
	status = HAL_I2C_Master_Transmit_DMA(&hi2c1, dev_addr << 1, buffer, len + 1);
	if (status != HAL_OK) {
		return status;
	}
